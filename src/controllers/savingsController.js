...
// Substitui a função 'exports.transaction' antiga por esta:
exports.transaction = asyncHandler(async (req, res) => {
    const { id } = req.params; // ID do Cofre
    const { childId, tipo, valor } = req.body; 

    const resultado = await savingsService.handleTransaction(id, childId, tipo, valor);
